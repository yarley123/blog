...
public class PoemController {
    ...
    public Result randomPoem() {
        return new Result(true, 200, "生成随机古诗成功", poemService.randomPoem(PoemConstant.RandomPoem));
    }

    @GetMapping("/acrosticPoem")
    public Result acrosticPoem(@RequestParam("words") String words) {
        return new Result(true, 200, "生成藏头古诗成功", poemService.acrosticPoem(words, PoemConstant.AcrosticPoem));
    }

    @GetMapping("/randomPoem2")
    public Result randomPoem2(@RequestParam("words") String words) {
        return new Result(true, 200, "给出部分信息的情况下，随机生成剩余部分古诗成功", poemService.randomPoem2(words,PoemConstant.RandomPoem2));
    }
